...
enyo.kind({
	...
	create: function() {
		...
		this.next();
		window.SundayData = enyo.global.SundayData;
		//console.log("window",window.SundayData);
	},
	next: function() {
		var test = enyo.TestSuite.tests[this.index++];
		if (test) {
			enyo.log("STARTING TEST SUITE ", test.prototype.kindName);
			this.createComponent({name: test.prototype.kindName, kind: enyo.TestReporter, onFinishAll: "next"}).runTests();
		} else {
...
enyo.kind({
	...
	*/
	...
	clearTimer: function() {
        ...
        assertEqual: function(name,a,b) {
                var passed =  a === b;
                this._assert(name, passed, "'"+a+"' did not equal '"+b+"'", arguments.callee.caller.toString());
        },
        assertNotEqual: function(name,a,b) {
                var passed =  a !== b;
                this._assert(name, passed, "'"+a+"' did equal '"+b+"'", arguments.callee.caller.toString());
        },
        assertTrue: function(name,passed) {
                this._assert(name, passed, "was false", arguments.callee.caller.toString());
        },
        assertFalse: function(name,notpassed) {
                this._assert(name, !notpassed, "was true", arguments.callee.caller.toString());
        },
        assertIsNull: function(name,a) {
                var passed =  a === null;
                this._assert(name, passed, "is not null", arguments.callee.caller.toString());
        },
        assertIsNotNull: function(name,a) {
                var passed =  a !== null;
                this._assert(name, passed, "is null", arguments.callee.caller.toString());
        },
        assertIn: function(name,a,b) {
                var passed =  a in b;
                this._assert(name, passed, "'"+a+"' is not in '"+JSON.stringify(b)+"'", arguments.callee.caller.toString());
        },
        assertNotIn: function(name,a,b) {
                var passed =  !(a in b);
		...
		//this.clearTimer();
		this.doFinish({results: this.results}); 
        },
...
enyo.kind({
	...
	updateTestDisplay: function(inSender, inEvent) {
		...
		var content = results.assertname + ": " + (results.passed ? "PASSED" : results.message) + " Execution Time: " + results.runtime + " ms";
		if (e) {
			...
			if (results.failValue) {
				content += results.name+ " : " + results.failValue;
			}
		}
